...
def cal_connectivity_from_points(points=None, radius=0.1, K=10, trajectory=None, least_edge_num=3, node_radius=None, mode='nn', GraphK=4, adaptive_weighting=True):
    ...
    return ii, jj, nn, weight # [2083], [2083], [2083], [512, 10]

###
def cal_connectivity_from_points_v2(points=None, radius=0.1, K=10, trajectory=None, least_edge_num=3, node_radius=None, mode='nn', GraphK=4, adaptive_weighting=True):
     # input: [T, Nv,3]
     # output: information of edges
     # ii : [Ne,] the i th vert
     # jj: [Ne,] j th vert is connect to i th vert.
    ...
    nn_dist, nn_idx, nns = ops.ball_query(points, points, K=10+1, radius=radius)
    ...
    _, nn_idx = torch.topk(nn_idx, k=10, dim=1, largest=True)
    nn_idx = (nn_idx - 1).abs()
