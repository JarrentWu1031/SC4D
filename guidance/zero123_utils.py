...
class Zero123(nn.Module):
    def __init__(self, device, fp16=True, t_range=[0.2, 0.98], model_key="ashawkey/zero123-xl-diffusers"):
        super().__init__()
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0):
        ...
        with torch.no_grad():
            ...
            if self.embeddings[0].shape[0] > 1:
            ...
            if self.embeddings[1].shape[0] > 1:
        ...
        loss = 0.5 * F.mse_loss(latents.float(), target, reduction='sum') / batch_size
